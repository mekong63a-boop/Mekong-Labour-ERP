...
const HobbiesTab = () => {
  ...
  const { data: items = [], isLoading } = useQuery({
    queryKey: ["hobbies", search],
    queryFn: async () => {
      let query = supabase.from("hobbies").select("*").order("name");
  ...
  const addMutation = useMutation({
    ...
    onError: () => toast.error("Không thể thêm sở thích"),
  });

  const updateMutation = useMutation({
    mutationFn: async ({ id, ...data }: { id: string; name: string }) => {
      const { error } = await supabase.from("hobbies").update(data).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["hobbies"] });
  ...
  const handleSubmit = () => {
    if (!formData.name) {
      toast.error("Vui lòng điền tên sở thích");
      return;
    }
    ...
    if (!editingId) {
      ...
      if (isDuplicate) {
        toast.error(`Sở thích "${formData.name}" đã tồn tại trong hệ thống!`);
        return;
      }
    }
    ...
    <div className="space-y-4">
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
        ...
        <ul className="mt-2 text-sm text-amber-700 space-y-1">
          <li>• Quản lý danh sách sở thích của học viên</li>
          <li>• Các sở thích này sẽ hiển thị trong dropdown khi thêm/sửa học viên</li>
        </ul>
      </div>
      ...
      {showAddForm && (
        <div className="bg-card border rounded-lg p-4 space-y-4">
          <h3 className="font-semibold">{editingId ? "Chỉnh sửa sở thích" : "Thêm sở thích mới"}</h3>
          <div className="flex items-center gap-4">
            <Input
              placeholder="Tên sở thích (VD: Đọc sách, Nghe nhạc...)"
              value={formData.name}
        ...
        <table className="w-full">
          <thead className="bg-muted/50">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-sm font-medium text-primary">Tên sở thích</th>
      ...
      <p className="text-sm text-muted-foreground">Tổng cộng: {items.length} sở thích</p>
    </div>
  );
};
