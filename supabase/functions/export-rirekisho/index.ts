...
// ====== Minimal XLSX generator (no external library) ======
// Generates a valid .xlsx file using ZIP + XML
...
<Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/sharedStrings" Target="sharedStrings.xml"/>
  ...
  // Build ZIP manually using Compression Streams API is complex.
  // Use a minimal ZIP builder instead.
...
serve(async (req) => {
  ...
  try {
    ...
    const addCell = (r: number, c: number, v: any) => {
    ...
    // Row 0: No
    addCell(0, 28, "No");
    addCell(0, 30, traineeCode);

    // Row 1: 履歴書 + date
    addCell(1, 0, "履歴書");
    addCell(1, 14, `${today.getFullYear()}年${today.getMonth() + 1}月${today.getDate()}日`);
    merges.push({ s: { r: 1, c: 0 }, e: { r: 1, c: 13 } });

    // Row 2-3: 氏名
    addCell(2, 0, "氏名");
    addCell(2, 1, "フリガナ");
    addCell(2, 3, profile.furigana || "");
    merges.push({ s: { r: 2, c: 3 }, e: { r: 2, c: 37 } });
    addCell(3, 1, "英字表記");
    addCell(3, 3, (profile.full_name || "").toUpperCase());
    merges.push({ s: { r: 3, c: 3 }, e: { r: 3, c: 37 } });

    // Row 4: 生年月日
    addCell(4, 0, "生年月日");
    addCell(4, 3, toJpDate(profile.birth_date));
    addCell(4, 9, "(年齢");
    addCell(4, 11, calcAge(profile.birth_date));
    addCell(4, 13, "歳)");
    addCell(4, 15, "性別");
    addCell(4, 19, profile.gender === "Nam" ? "男" : profile.gender === "Nữ" ? "女" : "");

    // Row 5: 出生地 / 婚姻
    addCell(5, 0, "出生地");
    addCell(5, 3, (profile.birthplace || "") + "省");
    addCell(5, 9, "(" + getRegion(profile.birthplace) + ")");
    addCell(5, 15, "婚姻");
    addCell(5, 19, profile.marital_status === "Độc thân" ? "未婚" : profile.marital_status === "Đã kết hôn" ? "既婚" : "");

    // Row 6: 現住所
    addCell(6, 0, "現住所");
    addCell(6, 3, profile.current_address || "");
    ...
    for (let i = 0; i < Math.min(eduHistory.length, 3); i++) {
      ...
      addCell(9 + i, 0, toYM(edu.start_year, edu.start_month));
      addCell(9 + i, 3, toYM(edu.end_year, edu.end_month));
      addCell(9 + i, 8, (edu.school_name || "") + "高校");
    ...
    for (let i = 0; i < Math.min(workHistory.length, 4); i++) {
      ...
      addCell(r, 0, sD ? `${sD.getFullYear()}年${sD.getMonth() + 1}月` : "");
      addCell(r, 3, eD ? `${eD.getFullYear()}年${eD.getMonth() + 1}月` : "現在");
      addCell(r, 8, (w.company_name || "") + "会社");
      merges.push({ s: { r, c: 8 }, e: { r, c: 23 } });
      const incomeStr = w.income ? `（月収${w.income}万円）` : "";
      addCell(r, 24, (w.position || "") + incomeStr);
      merges.push({ s: { r, c: 24 }, e: { r, c: 37 } });
    }

    // 過去の在留許可
    let cr = workStart + Math.max(workHistory.length, 1) + 1;
    addCell(cr, 0, "過去の在留許可申請・訪日経験");
    merges.push({ s: { r: cr, c: 0 }, e: { r: cr, c: 37 } });
    cr++;
    addCell(cr, 0, profile.prior_residence_status === "Có" ? "有" : "無");
    addCell(cr, 8, "目的 (                         )");
    ...
    for (let i = 0; i < Math.min(fam.length, 6); i++) {
      cr++;
      const fm = fam[i];
      addCell(cr, 0, relationMap[fm.relationship] || fm.relationship || "");
      addCell(cr, 3, fm.full_name || "");
      merges.push({ s: { r: cr, c: 3 }, e: { r: cr, c: 10 } });
      const age = fm.birth_year ? today.getFullYear() - fm.birth_year : "";
      addCell(cr, 11, age);
      addCell(cr, 17, fm.living_together ? "X" : "O");
      addCell(cr, 19, fm.occupation || "");
      merges.push({ s: { r: cr, c: 19 }, e: { r: cr, c: 30 } });
      addCell(cr, 31, fm.income ? `${fm.income} 万円` : "");
    }
    ...
    if (jr.length === 0) {
      ...
      for (let i = 0; i < Math.min(jr.length, 3); i++) {
        cr++;
        const rel = jr[i];
        addCell(cr, 3, rel.full_name || "");
        addCell(cr, 11, rel.age || "");
        addCell(cr, 17, rel.gender === "Nam" ? "男" : rel.gender === "Nữ" ? "女" : "");
        addCell(cr, 19, relationMap[rel.relationship] || rel.relationship || "");
        addCell(cr, 23, rel.residence_status || "");
        addCell(cr, 31, rel.address_japan || "");
      }
    }
    ...
    cr++;
    addCell(cr, 0, "身長");
    addCell(cr, 3, profile.height ? `${profile.height}cm` : "");
    addCell(cr, 8, "血液型");
    addCell(cr, 15, profile.blood_group || "");
    addCell(cr, 20, "メガネ" + (profile.glasses === "有" ? "有" : "無"));
    addCell(cr, 26, "左：");
    addCell(cr, 29, profile.vision_left != null ? `${profile.vision_left}/10` : "");
    addCell(cr, 33, "右：");
    addCell(cr, 35, profile.vision_right != null ? `${profile.vision_right}/10` : "");

    cr++;
    addCell(cr, 0, "体重");
    addCell(cr, 3, profile.weight ? `${profile.weight}kg` : "");
    addCell(cr, 8, "聴力");
    addCell(cr, 15, profile.hearing || "正常");
    addCell(cr, 20, "健康診断");
    addCell(cr, 26, profile.health_status || "異常なし");

    cr++;
    addCell(cr, 0, "利き手");
    addCell(cr, 3, profile.dominant_hand === "Phải" ? "右" : profile.dominant_hand === "Trái" ? "左" : (profile.dominant_hand || ""));
    addCell(cr, 8, "刺青");
    addCell(cr, 15, profile.tattoo ? "有" : "無");
    addCell(cr, 20, "Ｂ型肝炎");
    addCell(cr, 26, profile.hepatitis_b === "Có" ? "有" : "無");
    ...
    addCell(cr, 28, "趣味・特技");
    cr++;
    addCell(cr, 0, profile.japanese_certificate || "");
    merges.push({ s: { r: cr, c: 0 }, e: { r: cr, c: 27 } });
    addCell(cr, 28, profile.hobbies || "");
    merges.push({ s: { r: cr, c: 28 }, e: { r: cr, c: 37 } });

    // 生活態度
    cr += 2;
    addCell(cr, 0, "生活態度・学習態度・その他　[ 自己評価の場合は（　）で記載 ]");
    merges.push({ s: { r: cr, c: 0 }, e: { r: cr, c: 37 } });

    cr++;
    addCell(cr, 0, profile.drinking || "無");
    addCell(cr, 1, "飲酒 [ 多・少・無 ]");
    addCell(cr, 5, profile.smoking || "無");
    addCell(cr, 6, "喫煙 [ 多・少・無 ]");
    addCell(cr, 15, profile.gender_identity || "無");
    addCell(cr, 17, "性自認・指向の有無 [ 有・無・－ ]");
    addCell(cr, 25, profile.personality || "");
    addCell(cr, 27, "性格 [ 活・普・控 ]");

    cr++;
    addCell(cr, 0, profile.greeting_attitude || "");
    addCell(cr, 1, "あいさつ・受け答え [優・良・可]");
    addCell(cr, 5, profile.tidiness || "");
    addCell(cr, 6, "整理・整頓 [ 優・良・可 ]");
    addCell(cr, 15, profile.discipline || "");
    addCell(cr, 17, "規則の順守 [ 優・良・可・未 ]");
    addCell(cr, 25, profile.class_attitude || "");
    addCell(cr, 27, "授業態度 [ 優・良・可・未 ]");

    // 備考
    cr++;
    addCell(cr, 0, "備考");
    cr++;
    addCell(cr, 0, profile.rirekisho_remarks || "");
    merges.push({ s: { r: cr, c: 0 }, e: { r: cr, c: 37 } });
