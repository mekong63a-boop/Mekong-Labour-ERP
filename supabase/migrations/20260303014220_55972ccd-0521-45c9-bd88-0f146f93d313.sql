...
-- Thêm các cột mới vào bảng trainees (ẩn trên UI, chỉ dùng khi xuất 履歴書)
ALTER TABLE public.trainees
  ADD COLUMN IF NOT EXISTS glasses text,
  ADD COLUMN IF NOT EXISTS gender_identity text,
  ADD COLUMN IF NOT EXISTS personality text,
  ADD COLUMN IF NOT EXISTS greeting_attitude text,
  ADD COLUMN IF NOT EXISTS tidiness text,
  ADD COLUMN IF NOT EXISTS discipline text,
  ADD COLUMN IF NOT EXISTS class_attitude text,
  ADD COLUMN IF NOT EXISTS rirekisho_remarks text;

-- Thêm cột living_together vào family_members
ALTER TABLE public.family_members
  ADD COLUMN IF NOT EXISTS living_together boolean DEFAULT false;

-- Cập nhật RPC get_trainee_full_profile để thêm các trường mới
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        'start_month', eh.start_month,
        'end_year', eh.end_year,
        'end_month', eh.end_month
      )
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        'income', fm.income,
        'living_together', fm.living_together
      )
