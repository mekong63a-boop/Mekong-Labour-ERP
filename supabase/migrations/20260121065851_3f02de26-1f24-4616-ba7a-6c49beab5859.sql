...
BEGIN
  ...
  -- trainee_reviews schema differs: use created_at + rating/content, keep backward compatible keys
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'id', tr.id,
  ...
  -- test_scores schema differs: use test_name, keep older keys as NULL
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'id', ts.id,
