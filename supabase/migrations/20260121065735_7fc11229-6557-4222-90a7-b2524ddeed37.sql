...
BEGIN
  ...
  -- workflow (keep backward-compatible keys used by UI/PDF; fill missing fields with NULL)
  ...
  -- work history (NO responsibilities column)
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'id', wh.id,
  ...
  -- final result (use stable JSON keys expected by the app)
  result := jsonb_build_object(
    ...
    'cccd_place', CASE WHEN can_view_pii THEN trainee_record.cccd_place ELSE '***masked***' END,
    ...
    'parent_phone_1', CASE WHEN can_view_pii THEN trainee_record.parent_phone_1 ELSE '***masked***' END,
    'parent_phone_2', CASE WHEN can_view_pii THEN trainee_record.parent_phone_2 ELSE '***masked***' END,
    ...
    -- keep shape expected by UI even if tables not implemented
    'violations', '[]'::jsonb,
    'notes_history', '[]'::jsonb,
    'trainee_notes', '[]'::jsonb,
